...
function update(){if(!D||!storyActive||playMode!=="story")return;const n=node();if(n==="tutorial_1"||n==="dialogue_1")storyStage=0;else if(n==="tutorial_2"||n==="dialogue_2")storyStage=1;else if(n==="dialogue_3")storyStage=2;else storyStage=3;if(lastMap===TM&&MECHS.some(m=>m.kind==="gate"&&m.done)&&!gateSent){gateSent=true;state(s=>{s.flags.tutorialGateBroken=true;s.flags.tutorialExitUnlocked=true;s.counters.tutorialGateSpit=3;},"gate_broken");emit("FRAGILE_GATE_BROKEN",{gateId:"tutorial_fragile_gate"});}const exit=MS[TM]&&MS[TM].exits&&MS[TM].exits[0];if(lastMap===TM&&!exitSent&&gateSent&&exit&&inRect(snake.fx,snake.fy,exit.rect)){exitSent=true;emit("MAP_EXIT_ENTERED",{id:exit.id,mapId:TM,targetMap:exit.targetMap});}}
function tick(){if(!D||!storyActive||playMode!=="story")return;syncSignals();update();}
function interact(n){if(!D||!storyActive||playMode!=="story"||!n||n.kind!=="keti")return false;n.inside=true;if(node()!=="wilderness_keti_wait")return true;D.keti=n;state(s=>{s.actors.keti.met=true;},"keti_met");emit("ACTOR_INTERACTED",{actorId:"keti"});return true;}

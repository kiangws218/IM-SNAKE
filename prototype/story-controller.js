...
function setOwnership(m){const i=m&&m.interactions||{};ownership={actors:new Set(i.actors||[]),mechanics:new Set(i.mechanics||i.mechs||[]),exits:new Set(i.exits||[])};}
function targetId(target){return typeof target==="string"?target:target&&(target.storyId||target.id||target.kind)||"";}
function owns(kind,target){const key=kind==="actor"?"actors":kind==="mechanic"?"mechanics":kind==="exit"?"exits":"";return !!key&&ownership[key].has(targetId(target));}
function lvl(m){return{cleanMap:!!m.cleanMap,solidOutside:m.solidOutside||null,obstacles:m.obstacles||[],mechs:(m.gates||[]).map(g=>({kind:"gate",id:g.id,x:g.x,y:g.y,w:g.w,h:g.h,need:g.need||3,reward:null})),waves:[],caps:{slime:0,rat:0,flower:0},beans:0,npcs:[]};}
...
function loadMap(id,entry){
 const m=MS[id];lastMap=id;setOwnership(m);gateSent=false;exitSent=false;boss=null;bossStakes=[];playMode="story";storyStage=id===TM?0:6;reset(true);buildFromLevel(lvl(m));groundBeans.length=0;(m.beans||[]).forEach(p=>groundBeans.push({x:p[0],y:p[1]}));enemies.length=0;npcs.length=0;
 const sp=(entry&&m.entryPoints&&m.entryPoints[entry])||m.spawn;snake.fx=sp.x;snake.fy=sp.y;dir=sp.dir;snake.len=id===TM?CFG.initLen:CFG.initLen;coilPath();computeSegs();computeEnclosure();const rules=m.rules||{};curBeanTarget=Number(rules.beanTarget)||0;beanT=Number(rules.beanRespawn)||CFG.beanRespawn;for(let i=0;i<(Number(rules.initialRandomBeans)||0);i++)spawnBean();observedBeans=beansEaten;observedLen=snake.len;
...
function dialog(a){
 panelOpen=true;freeze=true;heldKeys.clear();fireKey=false;dirQueue.length=0;panelNpc=null;npcNode=null;const p=ensurePanel();p.style.display="flex";document.getElementById("npcName").textContent=a.speaker||"我";document.getElementById("npcSub").textContent=a.sub||"序章";drawPortraitInto(document.getElementById("npcPortrait"),a.portrait||"snake_self",!!a.crying);document.getElementById("npcSay").textContent=a.text||"";
...
function nameInput(){
 panelOpen=true;freeze=true;heldKeys.clear();fireKey=false;dirQueue.length=0;const p=ensurePanel();p.style.display="flex";document.getElementById("npcName").textContent="我";document.getElementById("npcSub").textContent="序章 · 输入名字";drawPortraitInto(document.getElementById("npcPortrait"),"snake_self",false);document.getElementById("npcSay").innerHTML="这段旅程，应该怎么称呼你？<br><input id='storyNameInput' maxlength='16' style='margin-top:12px;width:220px;padding:8px;background:#10111e;color:#ffe9b3;border:1px solid #ffd76e;border-radius:6px;font:14px Consolas'><button id='storyNameConfirm' class='npc-opt'>确定</button>";
...
function update(){if(!D||!storyActive||playMode!=="story")return;const n=node();if(n==="tutorial_1"||n==="dialogue_1")storyStage=0;else if(n==="tutorial_2"||n==="dialogue_2")storyStage=1;else if(n==="dialogue_3")storyStage=2;else storyStage=3;if(lastMap===TM&&MECHS.some(m=>m.kind==="gate"&&m.done)&&!gateSent){gateSent=true;state(s=>{s.flags.tutorialGateBroken=true;s.flags.tutorialExitUnlocked=true;s.counters.tutorialGateSpit=3;},"gate_broken");emit("FRAGILE_GATE_BROKEN",{gateId:"tutorial_fragile_gate"});}const exit=MS[TM]&&MS[TM].exits&&MS[TM].exits[0];if(lastMap===TM&&!exitSent&&gateSent&&exit&&inRect(snake.fx,snake.fy,exit.rect))routeInteraction("exit",exit,{type:"enter",apply:()=>{exitSent=true;emit("MAP_EXIT_ENTERED",{id:exit.id,mapId:TM,targetMap:exit.targetMap});}});}
function tick(){if(!D||!storyActive||playMode!=="story")return;syncSignals();update();}
function routeInteraction(kind,target,payload){if(!D||!storyActive||playMode!=="story"||!owns(kind,target))return false;if(kind==="actor"){target.inside=true;if(node()==="wilderness_keti_wait"){D.keti=target;state(s=>{s.actors.keti.met=true;},"keti_met");emit("ACTOR_INTERACTED",{actorId:targetId(target)});}return true;}if(payload&&typeof payload.apply==="function")payload.apply();return true;}
function interact(n){return routeInteraction("actor",n);}
...
root.openPanel=function(n,id){if(routeInteraction("actor",n))return;return old.open(n,id);};root.storyControllerInteract=interact;root.storyControllerInteraction=routeInteraction;root.storyControllerOwns=owns;root.storyControllerTick=tick;root.storyCheckProgress=update;root.storyGoalText=goal;
function start(){if(D)D.runtime.stop();ownership={actors:new Set(),mechanics:new Set(),exits:new Set()};storyActive=true;storyStage=0;storyTransitioning=false;playMode="story";const st=new S.StoryStateStore();st.reset("new_story");D={spawned:false,keti:null,runtime:new A.StoryRuntime({graph:G,state:st,bus:B,adapters:{load:a=>loadMap(a.id,a.entry),dialog:a=>dialog(a),banner:a=>banner(a.text),eatKeti:eatKeti,eatCorpse:eatCorpse,slimes:()=>{D.spawned=true;enemies.length=0;enemyAt({type:"slime",x:54.5,y:21.5});enemyAt({type:"slime",x:54.5,y:27.5});},freeExplore:()=>{close();banner("自由探索：30 秒后会出现记忆模糊");D.runtime.startTimer("free_explore_memory",30);},memoryBlur:()=>{state(s=>{s.flags.memoryBlurSeen=true;s.player.bodyLength=CFG.minLen;},"memory_blur");snake.len=CFG.minLen;computeSegs();dialog({speaker:"我",sub:"记忆模糊",text:"我……刚才吃了什么？脑子里只剩下一片空白。",choices:[{id:"name",label:"输入名字",next:"input_player_name"}]});},nameInput:nameInput}})};D.runtime.start("prologue_start");}
root.startStoryMode=start;root.startStoryStage=start;root.storyDirector=()=>D;root.startCampaign=()=>{boss=null;bossStakes=[];playMode="campaign";resetRun();carry={len:CFG.initLen,charges:1};levelIdx=0;startLevel();};document.getElementById("pauseHome").onclick=()=>root.showMenu();root.showMenu=()=>{if(D)D.runtime.stop();D=null;ownership={actors:new Set(),mechanics:new Set(),exits:new Set()};boss=null;bossStakes=[];storyActive=false;storyStage=-1;lastMap=null;return typeof old.menu==="function"?old.menu():null;};

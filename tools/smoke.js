...
try{
  ...
  if(!forestMap.terrain.some(terrain=>terrain.kind==="caveEntrance")||forestMap.exits[0].rect.join(",")!=="83,4,3,5"||caveMap.solidOutside.join(",")!=="2,8,60,17"||!caveMap.exits.some(exit=>exit.id==="cave_exit"))throw new Error("Forest cave mouth or PPT cave corridor geometry is missing");
  sandbox.__IMS.snake.fx=84;sandbox.__IMS.snake.fy=7;frames(2);
  ...
  sandbox.__IMS.enemies[0].hp=3;sandbox.__IMS.enemies[0].x=46.25;sandbox.__IMS.snake.fx=3;sandbox.__IMS.snake.fy=16;frames(2);sandbox.__IMS.snake.fx=84;sandbox.__IMS.snake.fy=7;frames(2);
  ...
  if(sandbox.storyDirector().runtime.node.id!=="cave_ajian_critical")throw new Error("Cave combat did not resolve: "+JSON.stringify({node:sandbox.storyDirector().runtime.node.id,enemies:sandbox.__IMS.enemies.length,panel:sandbox.__IMS.panelOpen,flags:sandbox.storyDirector().runtime.state.snapshot().flags,waiting:sandbox.storyDirector().runtime.waiting,spawned:sandbox.storyDirector().spawned}));
  chooseStory("使用回复药水");
